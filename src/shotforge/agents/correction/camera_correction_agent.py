...
class CameraCorrectionAgent(CorrectionAgent):
    ...
    def apply(self, state: ProjectState, plan: CorrectionPlan, target_version: int) -> CorrectionPatch:
        ...
        for shot_id in target_shot_ids(state, plan):
            camera_note = story_beat_upgrade(
                state,
                shot_id,
                "camera",
                localized_note(language, "agents.correction.camera.camera", plan, issues),
            )
            prompt_note = prompt_revision_note(
                state,
                shot_id,
                "camera",
                localized_note(language, "agents.correction.camera.prompt", plan, issues),
            )
            operations.append(
                operation(
        ...
        return CorrectionPatch(
            ...
            expected_effect=localized_note(language, "agents.correction.camera.effect", plan, issues),
            risk=plan.risk,
